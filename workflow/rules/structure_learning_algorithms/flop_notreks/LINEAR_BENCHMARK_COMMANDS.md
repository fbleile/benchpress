...
For the current threshold-only diagnostic, the support is exactly
`abs(W) >= --dagma-threshold`; no threshold search or edge-deletion repair is
applied. DAG and NOTREKS violations are reported independently, followed by
