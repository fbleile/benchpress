...
def test_manifest_and_command_file_are_unique_and_nonempty(tmp_path: Path) -> None:
    ...
    assert len({row["status_path"] for row in rows}) == len(rows)
    ...
    assert all("tools/cli.py run-config" in command for command in commands)
