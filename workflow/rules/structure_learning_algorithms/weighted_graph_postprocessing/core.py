...
def _repair(initial, strengths, scorer, checker, config, diagnostics):
    graph = initial.copy()
    # Follow one deterministic weakest-edge deletion path, stopping at the
    # first feasible graph.  This preserves the maximal feasible support;
    # BIC must not turn NOTREKS repair into an unrelated sparsity optimizer.
